...
const statusMeta: Record<InstallmentRow["status"], { label: string; color: string; icon: typeof Clock }> = {
  paid: { label: "Paid", color: "var(--success)", icon: CheckCircle2 },
  due: { label: "Due", color: "var(--primary)", icon: Clock },
  overdue: { label: "Overdue", color: "var(--danger, #ff5050)", icon: AlertTriangle },
    ...
    <div className="p-8 max-w-4xl">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        {[
          ...
          { label: "Remaining", value: usd(schedule.remainingBalance), color: "var(--warn, #eab308)" },
      ...
      <div className="rounded-xl border border-white/5 p-5 mb-6" style={{ background: "var(--surface)" }}>
        ...
        <div className="h-2 rounded-full" style={{ background: "var(--surface-low)" }}>
      ...
      <div className="rounded-xl border border-white/5 overflow-hidden" style={{ background: "var(--surface)" }}>
        <div className="grid grid-cols-[40px_1fr_1fr_1fr_1fr_90px] gap-2 px-4 py-2.5 text-[10px] font-bold uppercase tracking-widest border-b border-white/5" style={{ color: "var(--muted)" }}>
        ...
        <div className="max-h-[420px] overflow-y-auto">
          {schedule.rows.map((r) => {
            ...
            const Icon = m.icon;
            return (
              <div key={r.n} className="grid grid-cols-[40px_1fr_1fr_1fr_1fr_90px] gap-2 px-4 py-2.5 text-xs border-b border-white/5 last:border-0 tabular-nums items-center">
