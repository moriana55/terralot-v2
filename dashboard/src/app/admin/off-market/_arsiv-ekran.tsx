...
// ─────────────────────────────────────────────────────────────────────────────
// ARŞİV — eski "/admin/off-market" ekranının kodu. HİÇBİR YERDEN İMPORT EDİLMEZ,
// bu yüzden derlemeye girmez; referans olarak saklanıyor.
//
// Neden emekliye ayrıldı (2026-07-29 bilgi mimarisi toparlama):
//   • Kaynağı `lib/data.ts` → `offMarketProperties` ve o dizi BİLEREK boş (`= []`);
//     ekran boş tablo + "örnek veri" banner'ı gösteriyor, `avgDiscount` 0/0 → NaN%.
//   • Add / Eye / Edit / Trash butonlarının hiçbirinde `onClick` yok.
//   • Adı /admin/off-market-leads ve /admin/off-market-harita ile karışıyordu.
//   Yani kaybolan ÇALIŞAN bir işlev yok. Sayfa /admin/arsa-notlari'na yönlendirildi.
//
// Saklanma sebebi: access-code / visibility / expiresAt veri modeli fikri.
// Gerçek bir off-market tablosu eklendiğinde bu ekran buradan diriltilebilir.
// ─────────────────────────────────────────────────────────────────────────────
...
export default function ArsivOffMarketEkrani() {
