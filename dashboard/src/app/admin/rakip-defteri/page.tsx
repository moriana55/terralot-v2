...
export default function RakipDefteriPage() {
      ...
      {tab === "defter" ? (
        <>
          ...
          <div className="flex items-center gap-3 flex-wrap rounded-xl border p-4"
            ...
            <select value={statuFiltre} onChange={(e) => { setStatuFiltre(e.target.value as RakipDefteriKayitTipi | "hepsi"); setSayfa(0); }}
              className="px-3 py-2 rounded-lg text-xs font-semibold" style={{ background: "var(--surface-low)", border: "1px solid var(--surface-high)" }}>
              <option value="hepsi">Tüm statüler</option>
              <option value="dogrulanmis_satis">Satıldı ✓</option>
              <option value="satis_taksitte">Taksitte</option>
              <option value="envanter">Stokta</option>
              <option value="belirsiz">Bilinmiyor</option>
            </select>
            <select value={bolgeFiltre} onChange={(e) => { setBolgeFiltre(e.target.value); setSayfa(0); }}
              className="px-3 py-2 rounded-lg text-xs font-semibold max-w-[200px]" style={{ background: "var(--surface-low)", border: "1px solid var(--surface-high)" }}>
